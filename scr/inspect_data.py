...
for folder in tqdm(os.listdir(path), desc='전체진행'):
    for file in tqdm(os.listdir(f'{path}/{folder}'), desc='1개월중 진행'):
        ...
        confirm = confirm.append({'file':file, '총 레코드수':total_shape, '스포츠기사': num_sports, '누락수': target_shape, '누락율':target_shape/total_shape}, ignore_index=True)
        결측 = 결측.append(target, ignore_index=True)

        del temp, total_shape, num_sports, target, target_shape

confirm.to_csv('D:/users/Desktop/Junhwi/국토연구원/HPU_Model/파일별결측내역서.csv', index= False, encoding='utf-8-sig')
